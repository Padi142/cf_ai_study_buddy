...
export function CalendarEvents() {
  ...
  }, [loadCalendarFromAgent]);

  useEffect(() => {
    if (refreshKey > 0) {
      void loadCalendarFromAgent();
    }
  }, [refreshKey, loadCalendarFromAgent]);

  // Sync calendar entries to the agent whenever they change
  useEffect(() => {
    if (!isInitialized) return;

    const syncCalendarToAgent = async () => {
      try {
        await fetch(`${AGENT_URL}/agents/studybuddyagent/mlem/calendar`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ entries: calendarEntries }),
        });
      } catch (error) {
        console.error("Failed to sync calendar to agent:", error);
      }
    };

    void syncCalendarToAgent();
  }, [calendarEntries, isInitialized]);

  const groupedEvents = useMemo(() => {
    if (!calendarEntries) return [];
    return groupEventsByDate(calendarEntries);
  }, [calendarEntries]);
  ...
  const handleAddEvent = () => {
    ...
    const event: CalendarEntry = {
      ...
      type: newEvent.type as CalendarEntry["type"],
    };
